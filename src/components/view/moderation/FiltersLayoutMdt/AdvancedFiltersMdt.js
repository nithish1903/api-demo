...
const AdvancedFiltersMdt = () => {
  const { reviewFilter,handleContent_type,handleStatus ,handlePage}  = useReviewsFilter()
          ...
          <div className='grid grid-cols-12 gap-4'>
            {
              productTypes.map((prdTpy,p)=>{
                return  <div key={p} onClick={()=>{
                  handleContent_type(prdTpy.name)
                  handlePage(1)
                }} className={`col-span-6 xl:col-span-4 cursor-pointer ${prdTpy.name===reviewFilter.content_type?"bg-[#0266E1] border-[#0266E1]":"bg-[#fff] border-[#CFD5E1]"}  py-2.5 px-1.5 text-center rounded-[6px] border-2 `}>
          ...
          <div className='grid grid-cols-12 gap-2'>
            {
              statusTypes.map((statusType,s)=>{
                return <div onClick={()=>{
                  handleStatus(statusType.value)
                  handlePage(1)
                }} key={s} className={`col-span-6 xl:col-span-3 cursor-pointer ${statusType.value===reviewFilter.status?"bg-[#0266E1] border-[#0266E1]":"border-[#CFD5E1] bg-[#fff]"} py-2.5 px-1.5 text-center rounded-[6px] border-2`}>
